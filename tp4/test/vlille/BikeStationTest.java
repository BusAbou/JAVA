...
public class BikeStationTest {
   ...
   @Test
   public void numberOfBikesIsZeroAtCreation() {
      BikeStation station = new BikeStation("LilleCentre", 5);
      assertEquals(0, station.getNumberOfBikes(), "A newly created station should contain 0 bikes");
   }

   @Test
   public void numberOfBikesUpdatesCorrectlyAfterDropAndTake()  throws BikeNotAvailableException {
      BikeStation station = new BikeStation("LilleVlille", 3);
      Bike b1 = new Bike("A1", BikeModel.ELECTRIC);
      Bike b2 = new Bike("B2", BikeModel.TANDEM);

      // at the start: 0 bikes
      assertEquals(0, station.getNumberOfBikes());

      // we drop off a bike
      assertTrue(station.dropBike(b1));
      assertEquals(1, station.getNumberOfBikes(), "After dropping one bike, count should be 1");

      // we drop off a second bike
      assertTrue(station.dropBike(b2));
      assertEquals(2, station.getNumberOfBikes(), "After dropping another bike, count should be 2");

      // we remove a bike
      station.takeBike(1);
      assertEquals(1, station.getNumberOfBikes(), "After taking a bike, count should decrease by 1");
   }
}
